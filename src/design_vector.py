...
class DesignVector:
    ...
    def scaled(self, scale_factor: float) -> "DesignVector":
        """Returns a uniformly scaled copy of the design vector."""
        return type(self)(
            wing_span=self.wing_span * scale_factor,
            wing_chord=self.wing_chord * scale_factor,
            tail_arm=self.tail_arm * scale_factor,
            nose_length=self.nose_length * scale_factor,
        )
...
class ASBDesignVector(DesignVector):
    ...
    def make_airplane(
        ...
        *,
    ...
    ) -> tuple["asb.Airplane", float, float, float]:
        ...
        return airplane, float(self.wing_area), float(self.wing_chord), float(self.wing_span)
