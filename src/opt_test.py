...
pd_constraint = NonlinearConstraint(
    lambda x: x[DesignVector.opt_names().index("prop_pitch_in")] / x[DesignVector.opt_names().index("prop_diameter_in")],
    0.4,   # minimum P/D
    0.8,   # maximum P/D
)
...
def save_best_design_visualization(design: DesignVector) -> tuple[Path, Path]:
    ...
    scoring_design = replace(
        design,
        extra_shipping_containers=round(design.extra_shipping_containers) if ROUND_PAYLOAD else design.extra_shipping_containers,
    )
