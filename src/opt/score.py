...
class ScoringReferences:
    ...
    best_ground_weight_height_kg_in: float = GM_REFERENCE_SENSOR_MASS_KG * GROUND_DROP_HEIGHT_IN
    seconds_per_mission: float = SECONDS_PER_MISSION
    m3_deployment_time_s: float = 0.0
    m3_recovery_time_s: float = 0.0

    def __post_init__(self) -> None:
        values = (self.best_m2_weight_per_time_kg_s, self.best_m3_lap_weight_kg,
                  self.best_ground_weight_height_kg_in, self.seconds_per_mission)
        ...
        if any(not math.isfinite(value) or value < 0.0 for value in overheads):
            raise ValueError("Mission-3 deployment/recovery times must be finite and nonnegative.")
        if sum(overheads) >= self.seconds_per_mission:
            raise ValueError("Mission-3 deployment/recovery must fit in the flight window.")
...
def scoring_reference_values(refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> dict[str, object]:
    return {
        "flight_window_s": refs.seconds_per_mission,
        "mission_2": {"best_weight_per_time_kg_s": refs.best_m2_weight_per_time_kg_s,
                      "required_laps": M2_REQUIRED_LAPS},
        "mission_3": {"best_lap_weight_kg": refs.best_m3_lap_weight_kg,
                      "deployment_time_s": refs.m3_deployment_time_s,
                      "recovery_time_s": refs.m3_recovery_time_s},
        "ground": {"drop_height_in": GROUND_DROP_HEIGHT_IN,
                   "reference_sensor_weight_lb": 35.0,
                   "best_weight_height_kg_in": refs.best_ground_weight_height_kg_in},
    }


def gm_score(dv: DesignVector, refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> float:
...
def m1_score(lap_time_s: float, refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> float:
...
def m1_optimization_score(lap_time_s: float, refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> float:
    if not math.isfinite(lap_time_s) or lap_time_s <= 0.0:
        return 0.0
    return min(1.0, refs.seconds_per_mission / (M1_REQUIRED_LAPS * lap_time_s))


def m2_score(payload_mass_kg: float, lap_time_s: float,
             refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> float:
...
def m2_optimization_score(payload_mass_kg: float, lap_time_s: float,
                          refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> float:
    official = m2_score(payload_mass_kg, lap_time_s, refs)
    if official > 0.0:
        return official
    if not math.isfinite(lap_time_s) or lap_time_s <= 0.0 or payload_mass_kg <= 0.0:
        return 0.0
    completion = min(1.0, refs.seconds_per_mission / (M2_REQUIRED_LAPS * lap_time_s))
...
def completed_m3_laps(lap_time_s: float,
                      refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> int:
    if not math.isfinite(lap_time_s) or lap_time_s <= 0.0:
        return 0
    lap_window = refs.seconds_per_mission - refs.m3_deployment_time_s - refs.m3_recovery_time_s
    return int(lap_window // lap_time_s)


def m3_score(dv: DesignVector, lap_time_s: float,
             refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> float:
...
def m3_optimization_score(dv: DesignVector, lap_time_s: float,
                          refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> float:
...
def total_score(dv: DesignVector, lap_time_m1: float, lap_time_m2: float,
                lap_time_m3: float, m2_payload_mass_kg: float,
                refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> tuple[float, list[float]]:
    """Return GM + M1 + M2 + M3 using the existing sequential unlock flow."""
    gm = gm_score(dv, refs)
...
def total_optimization_score(dv: DesignVector, lap_time_m1: float, lap_time_m2: float,
                             lap_time_m3: float, m2_payload_mass_kg: float,
                             refs: ScoringReferences = DEFAULT_SCORING_REFERENCES) -> tuple[float, list[float]]:
    """Return relaxed progress while preserving the existing official unlock gates."""
    gm = gm_score(dv, refs)
