...
class ToplineConfig:
    ...
    battery_cell_count: int = 8
    optimize_battery_cell_count: bool = False
    battery_cell_count_bounds: tuple[int, int] = (8,8)
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.battery_cell_count_bounds) != 2:
            raise ValueError("battery_cell_count_bounds must contain two values.")
...
def _integrality_mask(config: ToplineConfig | None = None) -> np.ndarray:
    names = _optimizer_variable_names(config)
    return np.array(
        [name in {"extra_shipping_containers", "battery_cell_count"} for name in names],
        dtype=bool,
    )
...
def _design_vector_from_optimizer(
    ...
    base_count = len(DesignVector.bounds())
    expected_count = base_count + int(config.optimize_battery_cell_count)
    if len(values) != expected_count:
        raise ValueError(
            f"Optimizer vector must have length {expected_count}, got {len(values)}."
        )
    cell_count = (
        normalize_battery_cell_count(values[-1])
        if config.optimize_battery_cell_count
        else config.battery_cell_count
    )
    if (
...
def _normalized_design_distance(
    left: np.ndarray,
    right: np.ndarray,
    config: ToplineConfig | None = None,
) -> float:
...
def _duplicate_islands(
    ...
    for index in ordered:
        ...
        if any(
            _normalized_design_distance(
                champion, states[other]["best_vector"], config
            )
            <= radius
...
def _combined_island_result(
    ...
    *,
    ...
    for row in _top_candidate_rows():
        vector = np.asarray(
            [row[name] for name in _optimizer_variable_names(config)], dtype=float
        )
        key = tuple(float(value) for value in vector)
        if key in seen:
            continue
...
def _run_niching_islands(config: ToplineConfig) -> OptimizeResult:
    ...
    while any(state["remaining"] > 0 for state in states):
        ...
        duplicates = _duplicate_islands(states, config.niche_radius, config)
        for island, state in enumerate(states):
            NICHE_HISTORY.append(
                {
                    ...
                    **{
                        ...
                        for name, value in zip(
                            _optimizer_variable_names(config), state["best_vector"]
                        )
                    },
                }
            )
    ...
    ACTIVE_ISLAND = 0
    return _combined_island_result(
        states,
        nfev=total_nfev,
        nit=total_nit,
        config=config,
...
def _write_top_candidate_archive(
    output_dir: Path,
    config: ToplineConfig,
) -> tuple[Path, Path]:
        ...
        try:
            with _module_output_context(config):
                score, _ = cast(
                    tuple[float, list[float]],
                    score_aircraft(
                        _design_vector_from_optimizer(x, config),
                        PARAMETER_VECTOR,
                        disp_res=False,
                        round_payload=config.round_payload,
...
def _write_niche_champions(
    ...
    for index_value in order:
        ...
        if official_scores[index] <= -BAD_OBJECTIVE:
            continue
        if any(
            _normalized_design_distance(population[index], population[other], config)
            <= config.niche_radius
...
def _best_design_report(
    ...
    best_design = _design_vector_from_optimizer(best_vector, config)
    ...
    report = {
        ...
        "battery": {
            ...
            "nominal_energy_wh": scoring_design.batt_energy,
            "usable_energy_wh": (
                scoring_design.batt_energy * battery.useable_fraction
            ),
    ...
    summary = {
        ...
        "bounds": {
            ...
            for name, bounds in zip(
                _optimizer_variable_names(config), _optimizer_bounds(config)
            )
        },
        "integrality": {
            name: bool(flag)
            for name, flag in zip(
                _optimizer_variable_names(config), _integrality_mask(config)
            )
        },
    }
...
def run_topline_optimization(config: ToplineConfig | None = None):
    ...
    population_path = _write_final_population(
        result,
        official_scores,
        output_dir,
        config,
    ...
    payload_csv_path, payload_arrays_path = _write_payload_archive(
        output_dir, config
    )
    top_csv_path, top_arrays_path = _write_top_candidate_archive(
        output_dir, config
    )
