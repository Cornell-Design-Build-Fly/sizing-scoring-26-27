...
def _score_color_scale(scores):
    """Create a zero-based score scale with negative values shown in grey."""
    ...
    color_map.set_under("0.55")
    upper_limit = maximum if maximum > 0.0 else 1.0
    return color_map, Normalize(vmin=0.0, vmax=upper_limit, clip=False)
...
def plot_payload_scores(
    container_counts, scores, variable_names, bounds, *, save_path=None, show=True,
):
    """Plot the best observed score for each optional shipping-container count."""
    counts = np.asarray(container_counts, dtype=int)
    scores = np.asarray(scores, dtype=float)
    if counts.ndim != 1 or scores.shape != counts.shape:
        raise ValueError("Container counts and scores must be matching vectors.")
    lower, upper = bounds[variable_names.index("extra_shipping_containers")]
    best = {}
    for count, score in zip(counts, scores):
        if lower <= count <= upper and np.isfinite(score):
            best[count] = max(best.get(count, -np.inf), score)
    if not best:
        return
    counts = sorted(best)
    fig, axis = plt.subplots(figsize=(10, 6), constrained_layout=True)
    axis.bar(counts, [best[count] for count in counts])
    axis.set(xlabel="Extra shipping containers", ylabel="Best score",
             title="Best Score Seen for Each Container Count")
