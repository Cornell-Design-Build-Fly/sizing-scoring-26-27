...
class MechanicalModuleConfig:
    ...
    def __post_init__(self) -> None:
        if not np.isclose(self.sensor.diameter_m, SENSOR_DIAMETER_M):
            raise ValueError(
                "Mechanical sensor diameter must match the DesignVector's fixed "
                "3-inch cylinder diameter."
            )
        if not np.isclose(
            self.sensor.steel_density_kg_m3,
            SENSOR_STEEL_DENSITY_KG_M3,
        ):
            raise ValueError(
                "Mechanical sensor density must match the DesignVector's fixed "
                "steel density."
            )
        if not (
