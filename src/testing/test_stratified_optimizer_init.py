...
def test_full_range_population_is_reproducible_and_feasible() -> None:
    ...
    assert np.all(max_sensor_weight == config.sensor_weight_kg)
    assert tuple(bounds[names.index("sensor_weight_kg")]) == (
        config.sensor_weight_kg,
        config.sensor_weight_kg,
    )
    assert bounds[names.index("mission3_sensor_weight_kg"), 0] == 0.05
    assert np.ptp(takeoff_flap) > 35.0
    assert np.all(containers == 0)
