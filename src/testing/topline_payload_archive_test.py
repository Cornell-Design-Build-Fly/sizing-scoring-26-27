...
class PayloadArchiveTest(unittest.TestCase):
    def _vector(self, ducks: int, pucks: int, wing_span: float) -> np.ndarray:
        vector = np.array(
            [sum(bound) / 2.0 for bound in DesignVector.bounds()],
            dtype=float,
        )
        names = DesignVector.opt_names()
        vector[names.index("ducks_num")] = ducks
        vector[names.index("pucks_num")] = pucks
        vector[names.index("wing_span")] = wing_span
        return vector

    def test_keeps_only_best_design_for_each_payload_pair(self):
        archive = {}
        first_population = np.vstack(
            [
                self._vector(3, 1, 1.0),
                self._vector(3, 1, 1.1),
                self._vector(6, 2, 1.2),
                self._vector(9, 3, 1.3),
            ]
        )
        ...
        self.assertEqual(set(archive), {(3, 1), (6, 2)})
        self.assertEqual(archive[(3, 1)]["score"], 5.0)
        self.assertEqual(archive[(3, 1)]["wing_span"], 1.1)

        second_population = np.vstack(
            [
                self._vector(3, 1, 1.4),
                self._vector(6, 2, 1.5),
            ]
        )
        updates = _update_payload_archive(
            second_population,
        ...
        self.assertEqual(archive[(3, 1)]["wing_span"], 1.1)
        self.assertEqual(archive[(6, 2)]["score"], 6.0)
        self.assertEqual(archive[(6, 2)]["wing_span"], 1.5)
        self.assertEqual(archive[(6, 2)]["generation"], 2)
