...
pack resistance, and mechanical battery mass all use that same count. Normal
sizing runs are fixed at 8S. `DesignVector` also enforces the incoming 100 Wh
propulsion-battery limit, so capacity must decrease as cell count increases.
...
power, voltage sag, segment-resolved mission energy, takeoff, climb,
propulsion-limited turns, and propeller-tip-speed limits. Battery depletion is
integrated from current at nominal pack voltage, while motor power limits use
the sagged terminal voltage. It does not yet check ESC/full-charge voltage
compatibility or cell-count-dependent packaging volume. Treat comparison-mode
results as internally consistent sizing guidance until those assumptions are
validated against the selected hardware.
