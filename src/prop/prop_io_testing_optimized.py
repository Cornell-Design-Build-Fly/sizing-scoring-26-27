...
# Try exact and non-exact props here.
# Example exact-ish:
...
def main():
    print("=== Optimized Prop Section Input/Output Test ===")

    print("\n=== Loading optimized prop database ===")
    prop_database = load_default_optimized_prop_database()

    print("\nDatabase diameter bounds [in]:")
    print(prop_database.diameter_bounds_in)

    print("\nDatabase pitch bounds [in]:")
    print(prop_database.pitch_bounds_in)

    print("\n=== Creating prop model for requested diameter/pitch ===")
    ...
    print("\n=== Optimized Prop Section Outputs ===")
    ...
    print("\n=== MATLAB-style optimized outputs ===")
    ...
    assert np.allclose(p_throttled_thrust, result.throttled_thrust)
    assert np.allclose(p_max_thrust, result.max_thrust)
    assert np.allclose(p_throttled_t, result.throttled_time)
    assert np.allclose(p_max_t, result.max_time)

    print("Passed optimized prop input/output sanity checks.")
